...
function StorageNode({ id, data }: NodeProps) {
  ...
  return (
    <div className="node storage">
      <div className="node-title"><span className="icon">🗄️</span> Storage <button className="toggle" onClick={() => updateNodeData(id, { showDescription: !data?.showDescription })}>{data?.showDescription ? 'Hide' : 'Show'}</button></div>
